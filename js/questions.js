...
text_edit +=  waste + chaps_arr[i].replace(vs_text, "<b style='color: red; background:white' id='item'>"+vs_text+"</b>");
}else{
text_edit += chaps_arr[i];
}
}
...
var get_verse_text = function(bk, chap, vs){
bk = getBookDbd(bk);
var data = window[bk];
data = data.substring(data.indexOf("*")-0+1, data.length);
var chaps_arr = [];
...
var chapt = vrss_arr[0];
var vsno = vrss_arr[1];
var vs_text = get_verse_text(currBk, chapt, vsno);
var s = dashverse(vs_text, keywords);
displayQn(currQn, currDs, s);
...
var vrss = vrefs[currQn-1];
var vrss_arr = vrss.split(":");
var chapt = vrss_arr[0];
var vsno = vrss_arr[1];
var vs_text = get_verse_text(currBk, chapt, vsno);
var st = dashverse(vs_text, keywords);
